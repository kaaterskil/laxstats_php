...
function get_scoreboard_games($season, $month){
	...
	$sql = 'SELECT gm.gameRef, gm.date, gm.fieldRef, gm.seasonType,
				...
				IF(t2.town = f.town, SUM(IF(g.teamRef = t2.teamRef, 1, 0)), SUM(IF(g.teamRef = t1.teamRef, 1, 0))) AS home_score,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t2.teamRef AND g.quarter = 1, 1, 0)), SUM(IF(g.teamRef = t1.teamRef AND g.quarter = 1, 1, 0))) AS home_goalsQ1,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t2.teamRef AND g.quarter = 2, 1, 0)), SUM(IF(g.teamRef = t1.teamRef AND g.quarter = 2, 1, 0))) AS home_goalsQ2,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t2.teamRef AND g.quarter = 3, 1, 0)), SUM(IF(g.teamRef = t1.teamRef AND g.quarter = 3, 1, 0))) AS home_goalsQ3,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t2.teamRef AND g.quarter = 4, 1, 0)), SUM(IF(g.teamRef = t1.teamRef AND g.quarter = 4, 1, 0))) AS home_goalsQ4,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t2.teamRef AND g.quarter LIKE \'O%\', 1, 0)), SUM(IF(g.teamRef = t1.teamRef AND g.quarter LIKE \'O%\', 1, 0))) AS home_goalsOT,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t1.teamRef, 1, 0)), SUM(IF(g.teamRef = t2.teamRef, 1, 0))) AS visitor_score,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t1.teamRef AND g.quarter = 1, 1, 0)), SUM(IF(g.teamRef = t2.teamRef AND g.quarter = 1, 1, 0))) AS visitor_goalsQ1,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t1.teamRef AND g.quarter = 2, 1, 0)), SUM(IF(g.teamRef = t2.teamRef AND g.quarter = 2, 1, 0))) AS visitor_goalsQ2,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t1.teamRef AND g.quarter = 3, 1, 0)), SUM(IF(g.teamRef = t2.teamRef AND g.quarter = 3, 1, 0))) AS visitor_goalsQ3,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t1.teamRef AND g.quarter = 4, 1, 0)), SUM(IF(g.teamRef = t2.teamRef AND g.quarter = 4, 1, 0))) AS visitor_goalsQ4,
				IF(t2.town = f.town, SUM(IF(g.teamRef = t1.teamRef AND g.quarter LIKE \'O%\', 1, 0)), SUM(IF(g.teamRef = t2.teamRef AND g.quarter LIKE \'O%\', 1, 0))) AS visitor_goalsOT
			FROM games gm
			INNER JOIN sites f
...
//--------------------------------------------------/
//GET SCOREBOARD SHOT DATA
//--------------------------------------------------/
function get_scoreboard_shots($season, $month){
	global $db;
	$sql = 'SELECT gm.gameRef,
				IF(t2.town = f.town, SUM(IF(pl.teamRef = t2.teamRef, pl.shotsQ1 + pl.shotsQ2 + pl.shotsQ3 + pl.shotsQ4 + pl.shotsOT, 0)), SUM(IF(pl.teamRef = t1.teamRef, pl.shotsQ1 + pl.shotsQ2 + pl.shotsQ3 + pl.shotsQ4 + pl.shotsOT, 0))) AS home_shots,
				IF(t2.town = f.town, SUM(IF(pl.teamRef = t1.teamRef, pl.shotsQ1 + pl.shotsQ2 + pl.shotsQ3 + pl.shotsQ4 + pl.shotsOT, 0)), SUM(IF(pl.teamRef = t2.teamRef, pl.shotsQ1 + pl.shotsQ2 + pl.shotsQ3 + pl.shotsQ4 + pl.shotsOT, 0))) AS visitor_shots
			FROM games gm
			INNER JOIN sites f
				ON gm.fieldRef = f.fieldRef
			INNER JOIN plays pl
...
//--------------------------------------------------/
//GET SCOREBOARD GOALKEEPER DATA
//--------------------------------------------------/
function get_scoreboard_goalkeepers($season, $month){
	global $db;
	$sql = 'SELECT gm.gameRef, pm.reference, IF(pm.FName!= \'\', CONCAT(SUBSTRING(pm.FName, 1, 1), \'. \', pm.LName), pm.LName) AS name, p.teamRef
...
function get_schedule($start_month, $end_month){
	global $db;
	$sql = 'SELECT DISTINCT gm.gameRef, gm.date, gm.startTime, TIME_TO_SEC(gm.startTime) AS time_as_seconds, gm.fieldRef, gm.final, gm.created, gm.modified,
			...
			WHERE date >= \''.date('Y-m-d', $start_month).'\' AND date <= \''.date('Y-m-d', $end_month).'\'
			GROUP BY gm.gameRef
...
//--------------------------------------------------/
//GET SCHEDULE GOALIES
//--------------------------------------------------/
function get_schedule_goalies($gameRef){
	global $db;
	$sql = 'SELECT pm.reference, pm.LName, s.savedQ1 + s.savedQ2 + s.savedQ3 + s.savedQ4 + s.savedOT AS saves, s.teamRef
			FROM playerMaster pm
			INNER JOIN players p
				ON pm.reference = p.playerMasterRef
			INNER JOIN saves s
...
//--------------------------------------------------/
//SET SCOREBOARD GOALKEEPER DATA
//--------------------------------------------------/
...
//--------------------------------------------------/
//GET BOX SCORE GAME DATA
//--------------------------------------------------/
function get_box_score_game($gameRef){
	global $db;
	$sql = 'SELECT gm.season, gm.seasonType, gm.date, TIME_TO_SEC(gm.startTime) AS time, gm.usTeamRef, gm.themTeamRef, gm.fieldRef, gm.conditions, gm.scorekeeper, gm.timekeeper, gm.referee, gm.umpire, gm.fieldJudge, gm.conference,
...
function get_box_score_scoring($gameRef){
	...
	$sql = 'SELECT g.teamRef, g.quarter, g.timeClock, g.teamRef, g.goalCode,
				...
				IF(g.assist>0, IF(p2.FName!= \'\', CONCAT_WS(\' \', \'Assist:\', p2.FName, p2.LName), CONCAT_WS(\' \', \'Assist:\', p2.LName)), \'Unassisted\') AS assist
			FROM goals g
...
//--------------------------------------------------/
//GET BOX SCORE LINEUP
//--------------------------------------------------/
function get_box_score_lineup($teamRef, $gameRef){
	global $db;
	$sql = 'SELECT p.jerseyNo, IF(p.FName!= \'\', CONCAT_WS(\' \', p.FName, p.LName), p.LName) AS player_name,
				IF(pl.gameRef = "'.$gameRef.'", pl.position, p.position) AS position,
				IF(pl.gameRef = "'.$gameRef.'", IF(pl.position = \'A\' OR pl.position = \'M\' OR pl.position = \'\', 1, 2), IF(p.position = \'A\' OR p.position = \'M\' OR p.position = \'\', 1, 2)) AS sequence,
				IF(pl.gameRef = "'.$gameRef.'", pl.started, NULL) AS started
			FROM players p
			LEFT JOIN plays pl
				ON p.teamRef = pl.teamRef AND p.jerseyNo = pl.playerRef AND pl.gameRef = '.$gameRef.'
...
//--------------------------------------------------/
//GET BOX SCORE OFFENSIVE STATS
//--------------------------------------------------/
function get_box_score_offense($teamRef, $gameRef){
	global $db;
	$sql = 'SELECT pl.position, p.jerseyNo, p.playerMasterRef, IF(p.FName!= \'\', CONCAT_WS(\' \', p.FName, p.LName), p.LName) AS player_name,
				...
				IF(pl.shotsQ1 + pl.shotsQ2 + pl.shotsQ3 + pl.shotsQ4 + pl.shotsOT > 0, FORMAT(SUM(IF(pl.playerRef = g.scorer, 1, 0)) / (pl.shotsQ1 + pl.shotsQ2 + pl.shotsQ3 + pl.shotsQ4 + pl.shotsOT), 3), FORMAT(0, 3)) AS shot_pct,
			...
			ORDER BY points DESC, goals DESC, assists DESC, shot_pct DESC, shots DESC, gb DESC, p.jerseyNo';
	$r = $db->db_query($sql);
	return $r;
}

//--------------------------------------------------/
//GET BOX SCORE FACEOFF STATS
//--------------------------------------------------/
function get_box_score_faceoffs($teamRef, $gameRef){
	global $db;
	$sql = 'SELECT p.playerMasterRef, IF(p.FName!= \'\', CONCAT_WS(\' \', p.FName, p.LName), p.LName) AS player_name,
			...
			INNER JOIN plays pl
				ON fo.gameRef = pl.gameRef AND fo.teamRef = pl.teamRef AND fo.playerRef = pl.playerRef
...
function get_box_score_saves($teamRef, $gameRef){
	...
	$sql = 'SELECT p.playerMasterRef, IF(p.FName != \'\', CONCAT_WS(\' \', p.FName, p.LName), p.LName) AS player_name,
			...
			INNER JOIN plays pl
				ON sv.gameRef = pl.gameRef AND sv.teamRef = pl.teamRef AND sv.playerRef = pl.playerRef
